    ...
    for (let i = 0; i < toxinas.length; i++) {
      ...
      for (let j = 0; j < samples.length; j++) {
        ...
        displayInfo[i].results[j] = {
          ...
          average: (sample[toxinafull].absorbance + sample[toxinafull].absorbance2) / 2,
          number: sample.samplenumber,
          changed_workmap: j != 0 && samples[j - 1][toxinafull].workmapId != sample[toxinafull].workmapId, //CONFERIR ORDEM DOS SAMPLES NESSE OBJETO
          _id: sample._id
        };
      }
