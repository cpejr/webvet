...
function comparara(logb_bo_amostra,intercept,slope){
 ...
 router.get('/', auth.isAuthenticated,async function(req, res, next) {
  ...
  calib[0] = aflaArray[0].calibrators.P1.sampleID;
  calib[1] = aflaArray[0].calibrators.P2.sampleID;
  calib[2] = aflaArray[0].calibrators.P3.sampleID;
  calib[3] = aflaArray[0].calibrators.P4.sampleID;
  calib[4] = aflaArray[0].calibrators.P5.sampleID;
 // console.log(calib);

  Sample.getById(calib[0]).then((p1)=>{
    aflas_p[0]=p1;
    Sample.getById(calib[1]).then((p2)=>{
      aflas_p[1]=p2;
      Sample.getById(calib[2]).then((p3)=>{
        aflas_p[2]=p3;
          Sample.getById(calib[3]).then((p4)=>{
            aflas_p[3] = p4;
            Sample.getById(calib[4]).then((p5)=>{
                aflas_p[4]=p5;
 ...
 // console.log(aflas_p[0].aflatoxina.absorbance); //abs afla calibrator
  ...
  for (var i = 0; i < 4; i++){
    b_b0[cont_b0] = aflas_p[i+1].aflatoxina.absorbance/aflas_p[0].aflatoxina.absorbance;
    cont_b0++;
  }
 // console.log(b_b0);
