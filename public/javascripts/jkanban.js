  ...
  1: [function (require, module, exports) {
    ...
    (function () {
      this.jKanban = function () {
          ...
          if (window.innerWidth > self.options.responsive) {
            ...
            self.drake = self
              ...
              .on("drop", function (el, target, source, sibling) {
                ...
                if (el !== null) {
                  var result = self.options.dropEl(el, target.parentNode.dataset.id, source, sibling);
                  if (result === false) {
                    self.drake.cancel(true);
                  }
                  el.classList.remove("is-moving");
                  if (typeof el.dropfn === "function")
                    el.dropfn(el, target.parentNode.dataset.id, source, sibling);
                }
              });
          }
        };
        ...
        this.addFixedElement = function (boardID, element) {
          var board = self.element.querySelector(
            '[data-id="' + boardID + '"] .kanban-drag'
          );
        ...
        this.addElementStandart = function (boardID, element) {
          var board = self.element.querySelector(
            '[data-id="' + boardID + '"] .kanban-drag'
          );
          var nodeItem = document.createElement("div");
        ...
        function __onclickHandler(nodeItem) {
          nodeItem.addEventListener("click", function (e) {
            ...
            if (typeof nodeItem.clickfn === "function") nodeItem.clickfn(this);
          });
        }
